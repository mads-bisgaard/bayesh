...
compadd () {
    ...
    if [[ -z $hsuf && -z $asuf && "${${@//-default-/}% -# *}" == *-[[:alnum:]]#f* ]]; then
        dirsuf=1
    fi
    ...
    # display all matches
    local dsuf pre post
    pre=${LBUFFER%${(b)IPREFIX}${(b)PREFIX}}
    post=${RBUFFER#${(b)SUFFIX}${(b)ISUFFIX}}
    ...
    for i in {1..$#__hits}; do
        ...
        echo -E - $pre$IPREFIX$apre$hpre$__hits[$i]$dsuf$hsuf$asuf$ISUFFIX$post
